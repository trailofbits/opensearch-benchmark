...
get_doc_count() {
    ...
    # single expected doc count, or a min and max doc count
    ...
    doc_counts["vectorsearch"]="10000000 10005000" # min and max doc count to allow 5000 duplicates
...
check_value_range () {
    ...
    if (( $actual < $expected_min )); then
        printf 'Incorrect %s value, actual %s is less than expected min %s\n' "$name" "$actual" "$expected_min"
        exit 1
    fi

    if (( $actual > $expected_max )); then
        printf 'Incorrect %s value, actual %s is greater than expected max %s\n' "$name" "$actual" "$expected_max"
        exit 1
    fi
}
...
check_doc_count () {
    ...
    if [[ -z "$max_doc_count" ]]; then
        ...
        check_value_range "document count" "${DOC_COUNT:-$min_doc_count}" "${DOC_COUNT:-$max_doc_count}" "$doc_count"
    fi
}
...
check_params () {
    ...
    check_doc_count "$doc_count" "$workload"
