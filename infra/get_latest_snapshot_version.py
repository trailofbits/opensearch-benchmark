...
if latest_version is None:
    ...
    latest_version = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    print(f"Could not find a latest version, defaulting to {latest_version}", file=sys.stderr)

# Ensure that $LATEST_VERSION is in a date format '%Y-%m-%d_%H-%M-%S'
if not re.match(r'\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}', latest_version):
    print(f"Could not find a valid latest version (correct date format), error", file=sys.stderr)
    sys.exit(1)
