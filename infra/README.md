...
![layout image](fig/layout.svg "Layout")

This infrastructure is designed to be run from a user's host machine. Terraform and AWS CLI will provision resources to install search engines, run benchmark workloads, and gather results:

1. The user runs `terraform` to create virtual instances and install software required to run benchmarking on a workload.
1. `injest.sh` loads the workload's dataset onto the target instance (where the engine is installed) and stores a snapshot of the loaded data into an S3 bucket.
1. `restore_snapshot.sh` restores the snapshot of loaded data from the S3 bucket onto the target instance.
1. `benchmark.sh` runs OpenSearch Benchmark workloads on the target instance and stores the results into a Datastore instance.
1. Finally, the user can retrieve data from one of two places:

    * `get_results.sh` downloads the raw data directly from the load generator instance.
    * `report-gen` downloads OpenSearch Benchmark results in an organized human-readable format.
...
After you create an OpenSearch deployment to store results, you need to update the following terraform variables (see [Environment Setup](#environment-setup)):

* `datastore_host`: Data store host (do not include `https://`)
...
Specify the following `terraform.tfvars` variables (see [Environment Setup](#environment-setup)):
...
* Configure `terraform.tfvars`. Common variables to configure are:
  ...
  * `prefix_list_id`, `prefix_list_region`: See [Prefix List](#optional-prefix-list).
  * `snapshot_user_aws_access_key_id`: See [Snapshot S3 Bucket](#snapshot-s3-bucket).
  * `snapshot_user_aws_secret_access_key`: See [Snapshot S3 Bucket](#snapshot-s3-bucket).
  * `benchmark_environment`: Environment metadata tag for results in data store.
  * `datastore_host`, `datastore_username`, `datastore_password`: See [Data Store](#data-store).
  * `s3_bucket_name`: See [Snapshot S3 Bucket](#snapshot-s3-bucket).
